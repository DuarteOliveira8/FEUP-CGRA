...
class MyVehicleBody extends CGFobject
{
	constructor(scene)
	{
		...
		this.bottom = new MyUnitCubeQuad(this.scene,0,6,0,2); // vehicle's bottom structure
		this.top = new MyTrapezoid(this.scene, true, 0, 2, 0, 4); // vehicle's top structure (true = it has windows)
		this.end = new MyTrapezoid(this.scene, false, 0, 2, 0, 4); // vehicle's front and back structure (false = it doesn't have windows)
		this.rearViewMirror = new MyRearViewMirror(this.scene); // vehicle's two rear view mirrors
		this.headLights = new MyHemisphere(this.scene,30,12); // vehicle's front headlights
		this.frontWheels = new MyWheel(this.scene); // vehicle's frontwheels
		this.backWheels = new MyWheel(this.scene); // vehicle's backwheels
